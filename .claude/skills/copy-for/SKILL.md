...
- **Max length**: Discord messages cap at 2000 characters. Budget **1900 characters max** per message to leave margin for Unicode (emoji, special symbols count as more bytes than ASCII). Count with `len()` in Python or `wc -c` in bash. If the reformatted text exceeds 1900 characters, split at natural break points (paragraphs, section dividers) and separate parts with `--- SPLIT HERE ---` markers. Tell the user how many messages to paste.
...
- **No API/automation**: Articles must be created manually in X's web editor. Cannot be automated via API or browser automation.
...
2. Format the input text according to the platform rules above.
3. Write the formatted text and platform sidecar, then copy to clipboard:
...
cat /tmp/claude-copy-for.txt | pbcopy
```

4. Confirm to the user what was copied and for which platform. If the content was split (Discord), tell them how many parts.

## Important

- Do NOT regenerate or rewrite the content. Preserve the meaning and structure exactly — only change the formatting.
- Do NOT ask whether to copy to clipboard. Just do it.
- If the clipboard operation fails, show the formatted text in a code fence so the user can copy it manually.
- Do NOT add anything that wasn't in the original response (no headers, sign-offs, or commentary).
